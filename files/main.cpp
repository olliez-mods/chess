...
int main()
{
    sf::RenderWindow window(sf::VideoMode(sf::Vector2u(800, 800)), "SFML Chess", sf::Style::Titlebar | sf::Style::Close);
    ...
    while (window.isOpen())
    {
        ...
        {
            validMoveBox.setPosition(sf::Vector2f(validMoves[i].x*100, validMoves[i].y * 100));
        ...
        if (holdingPeice)
        {
            ...
            rect.setPosition(sf::Vector2f(mosPos.x - 50, mosPos.y - 50));
            window.draw(rect);
        }
        window.display();
    }
